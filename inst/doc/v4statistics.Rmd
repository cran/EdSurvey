...
vignette: >
  ...
  %\VignetteIndexEntry{4. Statistical Methods}
...
In some cases two means ($\mu_1$ and $\mu_2$) are calculated for potentially overlapping groups. When this is done 
The difference ($\Delta = \mu_1 - \mu_2$) may be of interest. The variance of this difference is then
estimated using the formula
$$\sigma_{\Delta}^2 = \sigma_1^2 + \sigma_2^2 - 2 *  p * \sigma_m^2$$
where a subscript of $m$ is used to indicate the subset that has more observations, $p$ is the ratio of the number of observations used in calculating both $\mu_1$ and $\mu_2$ divided by the $n$-size of the larger of the two samples.

When one group is a subset of the other, this is a part-whole comparison. The part-whole comparisons in the context of NAEP applies to comparisons between jurisdictions. Please use with caution when implementing this method to other types of gap comparisons. 
...
$$V_{imp,f}=\frac{m+1}{m(m-1)}\sum_{p=1}^m \left( \frac{\sum_{i \in \mathcal{A}_p} w_i }{\sum_{i \in \mathcal{U}_p} w_i } - \sum_{p'=1}^m \frac{\sum_{i \in \mathcal{A}_{p'}} w_i }{\sum_{i \in \mathcal{U}_{p'}} w_i } \right)^2$$
...
When a value does has plausible values, the jackknife covariance  ($\bm{C}(A,B)_J$) of two statistics $A$ and $B$ is estimated as 
$$\bm{C}(A,B)_J=\bm{C}_{jrr}(A,B) + \bm{C}_{imp}(A,B)$$

The sampling covariance is
$$\bm{C}_{jrr}(A,B) = \frac{1}{m^*} \sum_{i=1}^{m^*}  \sum_{j=1}^{J} (A_{jp} - A_{0p}) (B_{jp} - B_{0p}) \ . $$
where $A_jp$ is the estimate of $A$ using the $j$th jackknife replicate value and the $p$th plausible value, $A_{0p}$ is the estimate of $A$ using the full sample weights and the $p$th plausible value. This equation is the covariance analog to the variance equations presented above. 

The imputation variance is estimated according to the covariance analog to the
Rubin (1987) variance estimate:
$$\bm{C}_{imp}(A,B) = \frac{m+1}{m(m-1)} \sum_{p=1}^m (A_p - A_0)(B_p - B_0)$$
where $A_p$ is the estimate of $A$ using the $p$th set of plausible values and $A_0$ is the average of $A_p$ acorss the plausible values.

When the variable does not have plausible values $\bm{C}_{imp}(A,B)$ is zero.

Note that there is no Taylor series analog to this.
...
National Center for Education Statistics (2009). t *Test for Partly Overlapping Groups*. National Center for Education Statistics, online at [https://nces.ed.gov/nationsreportcard/tdw/analysis/2004_2005/infer_compare2_overlap.aspx](https://nces.ed.gov/nationsreportcard/tdw/analysis/2004_2005/infer_compare2_overlap.aspx).
