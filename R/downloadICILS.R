...
#' 
#' @param years an integer vector indicating the study year. Valid years are 2013 and 2018.
#' @author Tom Fink
#' 
...
downloadICILS <- function(years=c(2013, 2018)) {
  ...
  if(any(!(years %in% c(2013, 2018)))){
    stop(paste0("The argument ", sQuote("years"), " must have values of only ", sQuote("2013"), " or ", sQuote("2018"), "."))
  }
  
  linkURL <- "https://www.iea.nl/data-tools/repository"
  
  txt <- c()
  txt <- c(txt, paste0("Please manually download and extract the SPSS (*.sav) formatted ICILS 2013 or 2018 study data files from the IEA Data Repository to a folder on your local system or network. ",
  ...
  txt <- c(txt, paste0("\t", "4) Locate your downloaded zip file (*.zip) container and use an extraction program to extract the folder's file contents. ",
                       "It is recommended to extract the SPSS (*.sav) files to an easy-to-remember folder path based on the study and year (e.g., for Microsoft Windows OS, ", 
                       sQuote("C:/EdSurveyData/ICILS/2013/"), ", ", sQuote("C:/EdSurveyData/ICILS/2018/"), ")."))
