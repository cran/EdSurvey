...
parseSPSS_DATALIST <- function(spsTxt){
  ...
  outputList <- list(FileHandle = fileHandle,
                     RecordNumber = recN,
                     DataList = items)
...
parseSPSS_DATALIST_Items <- function(txt){
  ...
  #build output object
  df <- data.frame(VariableName = vars,
...
parseSPSS_VARIABLELABEL <- function(spsTxt, varNames, verbose = FALSE){
  ...
  if(anyDuplicated(tokenDF$varNames)){
    ...
    warning(paste0("Duplicate Variable Labels Defined For Variable(s): ", paste(sQuote(dupeVars), collapse = ","), 
...
parseSPSS_VALUELABEL <- function(spsTxt, varNames){
  ...
  retDF <- data.frame(variableName = tolower(varNames),
...
parseSPSS_MISSINGVALUES <- function(spsTxt, varNames){
  ...
  retDF <- data.frame(variableName = varNames,
