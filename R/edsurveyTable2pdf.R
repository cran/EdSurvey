...
edsurveyTable2pdf <- function(data, 
  ...
  if(! inherits(data,c("edsurveyTable", "edsurveyTableList")) ) {
    stop(paste0(sQuote("data"), 
                " must be an edsurveyTable or edsurveyTableList"))
  }
  
  preambleList <- c("\\documentclass[10pt]{article}",
  ...
  if (!all(all.vars(formula) %in% edsurveyTableVars)) {
    stop(paste0("Could not find variable ",
  ...
  for (i in 1:length(edsurveyTableDF)) {
    if (returnMeans) {
      edsurveyTableDF$textValue[i] <- paste(formatC(round(edsurveyTableDF$MEAN[i], 
                                                          digits = estDigits), 
                                                    format = 'f', digits = estDigits),
                                            " (", 
                                            formatC(round(edsurveyTableDF$'SE(MEAN)'[i], 
                                                          digits = seDigits),
                                                    format = 'f', digits = seDigits),
                                            ")",
                                            sep = "")
    } else {
      edsurveyTableDF$textValue[i] <- paste(formatC(round(edsurveyTableDF$PCT[i], 
                                                          digits = estDigits),
                                                    format = 'f', digits = estDigits),
                                            " (", 
                                            formatC(round(edsurveyTableDF$'SE(PCT)'[i], 
                                                          digits = seDigits),
                                                    format = 'f', digits = seDigits),
                                            ")",
                                            sep = "")
